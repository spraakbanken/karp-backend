...
class GenericPreviewEntry(search.PreviewEntry):
    ...
    def query(self, input_dto: search.PreviewEntryInputDto) -> search.EntryPreviewDto:
        with self.resource_uow:
            resource = self.resource_uow.repo.get_by_resource_id(
                input_dto.resource_id)

        entry_id = resource.id_getter()(input_dto.entry)

        lex.EntrySchema(resource.entry_json_schema).validate_entry(
            input_dto.entry)
        ...
        entry = self.entry_transformer.transform(
            input_dto.resource_id, input_entry)
