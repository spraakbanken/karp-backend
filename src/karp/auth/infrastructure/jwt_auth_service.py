...
_jwt_key = load_jwt_key(env("AUTH_JWT_PUBKEY_PATH", None))


def authenticate(credentials: str) -> User:
    logger.debug("authenticate called", extra={"credentials": credentials})

    try:
        jwt_decoded = jwt.decode(credentials, key=_jwt_key, algorithms=["RS256"], leeway=5)
        payload = JWTPayload(
            sub=jwt_decoded["sub"],
